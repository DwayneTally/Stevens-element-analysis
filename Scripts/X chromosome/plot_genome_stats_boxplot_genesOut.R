...
# Drop any groups not in the color map
...
outlier_table <- long_outliers %>%
  ...
  arrange(metric, group, desc(value))

write_csv(outlier_table, "genome_stats_outliers_by_superfamily.csv")

message("Outliers found:")
print(outlier_table)

# Plot
p <- ggplot(long_outliers, aes(x = group, y = value)) +
  geom_boxplot(
    aes(fill = group),
    outlier.shape = NA,
    alpha = 0.85
  ...
  geom_text_repel(
    data = long_outliers %>% filter(is_outlier),
    aes(label = species_file, color = group),
    size = 2.8,
    max.overlaps = 200,
    box.padding = 0.25,
    point.padding = 0.15,
...
ggsave(
  "genome_stats_boxplots_by_superfamily_colored_labeled_outliers.pdf",
  p,
  width = 14,
  height = 8
)
...
ggsave(
  "genome_stats_boxplots_by_superfamily_colored_labeled_outliers.svg",
  p,
  width = 14,
  height = 8,
  dpi = 300
...
message("Wrote genome_stats_boxplots_by_superfamily_colored_labeled_outliers.pdf and .svg")
message("Wrote genome_stats_outliers_by_superfamily.csv")
