...
if (length(args) < 2) {
  stop("
Usage:
  Rscript parse_blk_genomes_v4.R phasedBlks.csv genome_stats_by_species.csv
")
}
...
#For each (pair, blkID, chr) compute union bp
#Sum across chr to get per-(pair, blkID) length
#Median across blkIDs within the pair
...
#union bp per blkID per chr (genome1)
blk1_chr <- dt[, .(
  bp = interval_union_bp(s_genome1, e_genome1)
...
#union bp per blkID per chr (genome2)
blk2_chr <- dt[, .(
  bp = interval_union_bp(s_genome2, e_genome2)
